  ...
  DissectController.prototype = {
    dissect: async function() {
      try {
        ...
        await fse.ensureDirSync(path.join(
          this.config.dest,
          `/views-${this.config.target}/admin/${this.nameLowerCaseSingularCap}`,
        ));
    ...
    exportVue: async function(exportFilePath) {
      try {
        const filePath = path.join(appRoot, '/tmpl/view/vue.ejs');
    ...
    exportShow: async function(exportFilePath) {
      try {
        const filePath = path.join(appRoot, '/tmpl/view/show.ejs');
        const isExist = await fs.existsSync(exportFilePath);
        if (!isExist)
    ...
    exportCreate: async function(exportFilePath) {
      try {
        const filePath = path.join(appRoot, './tmpl/view/create.ejs');
        const isExist = await fs.existsSync(exportFilePath);
        if (!isExist)
          await fse.copySync(filePath, exportFilePath);
      } catch (e) {
    ...
    exportEdit: async function(exportFilePath) {
      try {
        const filePath = path.join(appRoot, '/tmpl/view/edit.ejs');
        const isExist = await fs.existsSync(exportFilePath);
        if (!isExist)
          await fse.copySync(filePath, exportFilePath);
      } catch (e) {
    ...
    exportIndex: async function(exportFilePath) {
      try {
        const filePath = path.join(appRoot, '/tmpl/view/index.ejs');
        const str = await fs.readFileSync(filePath, 'utf8');
        const buffer = ejs.render(str, this);
        const isExist = await fs.existsSync(exportFilePath);
        if (!isExist)
    ...
    exportForm: async function(exportFilePath) {
      try {
        const filePath = path.join(appRoot, '/tmpl/view/form.ejs');
        const str = await fs.readFileSync(filePath, 'utf8');
        const buffer = ejs.render(str, this);
        const isExist = await fs.existsSync(exportFilePath);
        if (!isExist)
