    ...
    DissectController.prototype = {
      dissect : async function() {
        try {
          ...
          await fse.ensureDirSync(path.join(this.config.dest, `/view-${this.config.target}/admin/${this.nameLowerCaseSingularCap}`));

          await this.exportVue(path.join(
            config.dest,
            `/view-${this.config.target}/admin/${this.nameLowerCaseSingularCap}/vue.ejs`,
          ));

          await this.exportShow(path.join(
            config.dest,
            `/view-${this.config.target}/admin/${this.nameLowerCaseSingularCap}/show.ejs`,
          ));

          await this.exportCreate(path.join(
            config.dest,
            `/view-${this.config.target}/admin/${this.nameLowerCaseSingularCap}/create.ejs`,
          ));


          await this.exportEdit(path.join(
            config.dest,
            `/view-${this.config.target}/admin/${this.nameLowerCaseSingularCap}/edit.ejs`,
          ));

          await this.exportIndex(path.join(
            config.dest,
            `/view-${this.config.target}/admin/${this.nameLowerCaseSingularCap}/index.ejs`,
          ));

          await this.exportForm(path.join(
            config.dest,
            `/view-${this.config.target}/admin/${this.nameLowerCaseSingularCap}/form.ejs`,
          ));
