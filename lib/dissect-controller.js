  ...
  const fs = require('fs')
    ...
    const DissectController = function(scaffold, model, config) {
      try {
        ...
        console.log('this.model.isServerSidePaging=>',this.model.isServerSidePaging);
    ...
    DissectController.prototype = {
      dissect : async function() {
        try {
          ...
          await this.exportRoute(path.join(config.dest, '/routes.js'));
      ...
      exportRoute: async function(exportFilePath) {
        try {
          const filePath = path.join(__dirname, '/tmpl/routes.ejs');
      ...
      exportAdminController: async function(exportFilePath) {
        try {
          const filePath = path.join(__dirname, '/tmpl/defaultAdminController.ejs');
          const str = await fs.readFileSync(filePath, 'utf8');
          const buffer =  ejs.render(str, this);
          await fs.writeFileSync(exportFilePath, buffer);
        } catch (e) {
      ...
      exportAdminApiController: async function(exportFilePath) {
        try {
          const filePath = path.join(__dirname, '/tmpl/defaultAdminApiController.ejs');
          const str = await fs.readFileSync(filePath, 'utf8');
          const buffer =  ejs.render(str, this);
          await fs.writeFileSync(exportFilePath, buffer);
        } catch (e) {
